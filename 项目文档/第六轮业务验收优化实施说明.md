# HealthDoc 第六轮业务验收优化实施说明（schema v12）

> 文档状态：第六轮当前实现与验收契约，更新于 2026-08-04。
> 历史说明：[`第六轮Agent升级实施说明.md`](第六轮Agent升级实施说明.md) 记录 schema v11 的 Agent 增量，作为历史文档保留，不代表当前数据库基线。
...
连续切换最多三层；链内账号不能重复，因此不能形成环。每次请求重新校验：
...
预约卡展示业务时间线：预约成功、到检、报告上传确认、待复核、报告发布，以及取消/机构关闭等终止节点。正式预约列表按当前有效账号作为受检者查询；代预约组单列为回执，不能借此进入目标用户健康详情。

健康趋势的“异常提示”同时聚合日常测量和已发布体检指标。每个指标默认展示筛选范围内最近一次异常，可展开全部异常，字段包括健康方向、指标、偏高/偏低/阳性/异常、数值、单位、日期、来源、参考范围和详情入口。若后续最新值正常，显示“历史异常，最新已恢复正常”。异常提示仅辅助定位，不替代医生诊断。
...
用户和同机构其他分院只能读取 `published`；同机构兄弟分院读取详情或附件继续写访问审计。通用生产迁移不会改动用户手工填写的医生姓名，缺失值仍由 DTO 显示“历史报告未记录”；本轮仅精准替换验收库中的两组旧占位值。验收数据使用固定的预置展示姓名，代码与校验器明确限定其用途，界面按正常业务姓名展示：
...
升级后机构不能继续处理。关键动作通过站内通知，存在可用邮箱时进入邮件 outbox。投诉 DTO 不返回报告正文、健康指标等无关健康信息。关联账号场景以当前有效账号作为投诉人，真实操作者进入委托审计。非法状态转换统一返回 `COMPLAINT_STATE_CONFLICT`。
...
本轮后端、前端、构建、浏览器、依赖审计、v12 数据强校验及 openGauss 迁移/幂等/备份恢复演练已于 2026-08-04 收口，命令与真实统计统一见《测试报告》。线上 Git SHA、冷备份目录、发布版本和冒烟结论仍须在真实发布完成后回填，旧 schema v10/v11 记录不能替代本轮门禁。
...
本轮已明确为演示环境全量替换时，使用 `-SyncDemoDatabase` 同步经 `validate_v12_demo.py` 校验的 SQLite 快照，并同时同步匹配媒体。服务器保留 SMTP、JWT、机构凭据加密密钥等环境配置。远端仍执行 v12 迁移校验，健康检查必须同时返回目标 SHA 和 `schema_version=12`。
