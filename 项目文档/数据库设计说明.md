...
本项目数据库物理实现以华为云 GaussDB（openGauss 生态）为运行库，数据库名为 `health_system`。所有业务表通过 SQLAlchemy 模型生成 DDL 并落库为普通关系表，当前没有单独设计分区表、物化视图、独立表空间或手工指定数据文件位置，表数据由 GaussDB 按默认存储方式统一管理。数据库对象创建入口在 `backend/app/__init__.py` 的 `db.create_all()`，基础机构、套餐、指标字典和默认管理员数据由 `backend/app/seed.py` 初始化。

各表统一采用整型 `id` 作为主键，主键值由数据库自增序列维护，便于外键引用和连接查询。字段类型按照数据特征选择：用户名、机构名、套餐名、指标编码等使用定长上限的 `String`；评论内容、机构描述和 OCR 原始文本使用 `Text`；套餐价格、指标参考范围使用 `Numeric(10,2)`；体检日期使用 `Date`，创建时间使用 `DateTime(timezone=True)`；可见状态、授权状态、异常标记使用 `Boolean`；指标别名使用 `JSON` 保存列表结构。体检报告文件不直接以大对象形式存入数据库，库中只保存 `report_file_url` 和 OCR 文本等元数据，实际上传文件由后端文件目录保存。

索引设计以业务查询路径为依据。所有主键自动形成主键索引；`users.username`、`users.email`、`indicator_dicts.code`、机构分院、机构套餐、亲友关系、档案指标等唯一约束对应唯一索引，用于保证候选键不重复。常用过滤和关联字段如 `owner_id`、`institution_id`、`record_id`、`indicator_dict_id`、`category_id`、`exam_date` 等在模型中设置索引，支持按用户查询健康档案、按机构查询评论与套餐、按日期和指标生成趋势分析。当前数据规模较小，尚未额外建立组合查询索引；后续若趋势分析或评论列表数据量增大，可再根据慢查询补充复合索引。

完整性控制也属于本项目物理设计的重要部分。外键约束保证用户、机构、套餐、档案、指标和评论之间的引用有效；CHECK 约束限制角色、状态、评分、价格、来源类型和非空文本等基础规则；触发器用于实现跨表规则，包括体检套餐必须属于对应机构、用户评论前必须有该机构体检档案、指标写入时自动维护异常标记。数据库连接由 Flask-SQLAlchemy 连接池复用，默认基础连接数为 `10`，最大溢出连接数为 `20`，在保证演示和开发并发访问的同时避免频繁创建数据库连接。
