...
表记录采用行式存储，一行保存一条用户、机构、档案、指标或评论记录。主键统一使用整型 `id`，由数据库序列生成，避免使用较长业务字段作为主键。字段落库类型以常规关系型类型为主：定长上限文本对应 `VARCHAR`，较长说明和 OCR 结果对应 `TEXT`，价格和参考范围对应 `NUMERIC(10,2)`，日期时间对应 `DATE` / 带时区时间戳，布尔状态对应 `BOOLEAN`，指标别名对应 `JSON`。体检报告原文件没有作为 BLOB 或大对象写入数据库，库内只保存文件路径和 OCR 文本，文件本体由后端上传目录保存，从而减少数据库数据页和备份体积。

从存储增长角度看，`institutions`、`packages`、`indicator_categories`、`indicator_dicts` 属于基础数据表，数据量较小且变化较少；`users`、`friend_relations`、`health_records`、`comments` 属于业务数据表，会随用户使用逐步增长；`health_indicators` 是健康档案的明细表，通常会随档案数量增长最快。当前项目数据规模较小，文档中迁移校验数据量也处于演示级别，因此没有按 `exam_date`、`owner_id` 等字段做水平分区，也没有为冷热数据设计独立存储区域。

索引是本项目主要的辅助存储结构。主键和唯一约束会在数据库中形成 B-tree 唯一索引；模型中设置 `index=True` 的外键或查询字段会形成普通二级索引，例如 `owner_id`、`institution_id`、`record_id`、`indicator_dict_id`、`category_id`、`exam_date`、评论用户和评论机构等字段。这些索引会占用额外存储空间，但可以减少按用户、机构、档案和日期查询时的全表扫描。当前未额外创建哈希索引、全文索引、位图索引、聚簇索引或多列复合索引，整体物理结构保持为“普通行存表 + 序列 + B-tree 索引”的简化方案。
