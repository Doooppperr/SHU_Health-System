...
- 公共知识使用 `BAAI/bge-small-zh-v1.5` 的 512 维本地向量和 Qdrant Local。`RAG_EMBEDDING_THREADS` 默认 `1`，避免在 4GB 服务器上因 ONNX 并行线程过多产生内存交换；资源充足时可显式调高。访客过滤为 `public`，登录普通用户可额外检索 `authenticated` 医学白名单；管理员不能附带健康档案。
