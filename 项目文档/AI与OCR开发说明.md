...
> 文档状态：融合 1.0—6.0 的 AI、Agent、OCR 与 RAG 设计说明。当前运行版本为 6.0/schema v11，更新于 2026-07-29。
...
当前自动化覆盖 AI 持久档案上下文、语义选择、趋势扩展、重试与数据权限、角色隔离、RAG 稳定切分/同步/重排/缓存降级、OCR Mock、草稿指标、锁定删除临时文件、机构范围隔离及发布后数据可见性。外部服务凭据不进入自动化测试。

## 11. schema v11 Agent 补充

第六轮在保留本章全部 `/api/ai` 契约的同时新增 `/api/agent`。DeepSeek 通过严格 Pydantic Schema 选择档案、趋势、机构、套餐、容量、预约状态及四类草稿工具；代码负责计算与权限，模型只规划和解释。Agent SSE 增加 `plan/tool_started/tool_completed/evidence/approval_required`，写操作经第一方确认后才提交。
