...
> 文档状态：融合历史 AI/Agent/OCR/RAG 与第八轮混合搜索的当前说明。运行基线为 schema v13，更新于 2026-08-07。Agent 可创建待付款订单，但不能代表用户完成付款。

## 第八轮目录意图理解

目录搜索的主召回始终由确定性代码完成。只有复杂自然语言或内容置信度不足时，才将最长 80 字的单次查询发给 DeepSeek，要求返回固定 JSON：性别/人群、健康方向、位置、套餐、预算及简短摘要。模型不接收用户 ID、个人资料、健康档案、联系方式或历史搜索，不拥有数据库查询工具，也不能向结果中添加平台不存在的机构或套餐。

解析结果经 Pydantic 结构校验后仅用于现有候选重排。连接/读取超时分别为 2/4 秒；未配置、超时、429 或非法结果均返回 `content_fallback`。目录 AI 使用独立访客/登录态限流及有界 TTL 查询缓存，避免重复输入反复调用模型；缓存只保存规范化查询与受控意图，不持久化到数据库。
