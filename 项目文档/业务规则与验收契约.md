# HealthDoc 业务规则与验收契约（schema v13）

> 文档状态：schema v13 当前业务与验收契约，更新于 2026-08-09。当前运营修复无 schema 迁移。

## 0B. 机构身份、画像样本与备案契约

1. 机构账号创建后不得自行修改所属主体、分院/门店名称、所在区域和详细地址；前端字段只读，后端对差异载荷返回 400。平台管理员保留纠错权限。
2. 咨询电话、交通信息、分机号、轮休日、分院简介和图库仍由当前绑定机构账号维护，机构 ID 始终取自登录绑定。
3. 画像验收样本必须是不可登录的明确演示主体，只进入去标识化聚合；新增脚本按固定标记幂等执行，不删除或修改线上用户、预约、报告和资金事实。
4. `healthdoc.cn`、`www.healthdoc.cn` 和原公网 IP 必须可访问同一发布 SHA；域名证书自动续期。页面备案号必须原文展示并链接工信部官网。

## 0A. 第八轮智能搜索契约

1. 三个目录接口不传 `search_mode` 时保持旧客户端响应兼容；传 `content|hybrid` 时增加 `search` 元数据、公开安全的匹配套餐与理由。
2. 候选只来自启用机构、启用且未运营暂停的分院、在售且当前生效的套餐；预约接口还必须继续满足日期、容量、权限与套餐生效规则。
3. 排序依次强调精确机构/分院名、套餐名、明确位置、套餐适用人群/健康方向、服务特点和描述；直接证据高于模型推断，同分按已有 ID 稳定排序。
4. 模型只接收最长 80 字的本次搜索词，不接收账号、个人资料、健康档案或历史查询；模型输出只作为受控重排条件，不能创造机构或套餐。
5. 未配置、超时、429、非法 JSON 等异常不使目录失败，接口以 HTTP 200 返回内容召回并标记 `content_fallback`；搜索历史不落库。

## 0. 第七轮资金契约

1. 提交预约只创建待付款订单并暂占 15 分钟；付款成功才把个人预约从 `pending_payment` 转为 `unfulfilled`，超时转为 `payment_expired` 并释放容量。
2. 付款订单状态固定为 `pending/paid/partially_refunded/refunded/expired`；逐人资金状态固定为 `pending/held/scheduled/settled/refund_required/refunded`。
3. 每个逐人明细按人民币两位小数、`ROUND_HALF_UP` 计算：服务费为金额的 2.5%，机构净额为 97.5%。
4. 报告 `published` 且锁档后安排七个自然日结算；存在未解决投诉时到期任务保持冻结。
5. 正常到检前取消全额原路退款；未到检不自动退款。投诉与退款仅原付款人可发起，每个个人预约至多一个案件。
6. 托管期退款冲减平台托管全额；结算后退款冲减机构净额和平台服务费，付款人始终收到订单全额。
7. 管理员裁决为 `institution_fault_refund` 或 `no_refund`。已结算且支持退款时产生准确 72 小时机构义务；逾期暂停预约及运营写操作，但保留登录、账号安全、通知、投诉和财务退款能力；全部义务完成后自动恢复。
8. 付款、过期、结算、退款均使用事务、状态条件更新、行锁和幂等键；worker 与关键读接口执行补偿扫描。
...
schema v13 在原健康管理、机构履约和 Agent 能力上统一交付：
...
- 数据库、验收数据、发布脚本和健康检查统一到 schema v13。
...
| 验收主题 | 当前契约位置 | 最终证据位置 |
|---|---|---|
| 实名认证与全局写守卫 | 第 2、5 节 | 《测试报告》schema v13 门禁 |
...
| schema v13 增量、迁移、验收数据与回滚 | 第 13—15 节 | 迁移/校验器及 openGauss 演练记录 |
...
### 13.1 schema v13 物理增量

- 新表：`payment_orders`、`payment_order_items`、`refund_cases`、`finance_transactions`、`finance_ledger_entries`；
- `appointments.status` 增加 `pending_payment/payment_expired`；
- `institutions` 增加独立运营暂停时间和原因；
- v12 的 `fulfilled` 预约按成交时套餐价回填历史已结算订单，回填时间使用升级时点；
- 内部账本主键和其他用户资料不进入预约、投诉或财务公开 DTO。

## 14. v12→v13 迁移与回滚

SQLite 使用 `backend/scripts/upgrade_local_database.py` 原子升级；openGauss/GaussDB 使用 `backend/scripts/migrate_schema_v13.py` 和 Alembic 修订 `backend/migrations/versions/20260804_schema_v13.py`。

迁移规则：

1. 新建资金表和运营暂停字段，保留全部 v12 业务行；
2. 扩展预约状态约束，旧预约状态保持原值；
3. 仅 `fulfilled` 且有报告的历史预约回填已付款、已结算订单；
4. 未完成、取消和无报告的旧预约继续显示“历史预约，无在线付款记录”；
5. SQLite 在同目录临时副本完成结构、完整性和外键校验后原子替换；openGauss 在冷备份后执行 Alembic 增量；
6. 回滚只允许发生在公开提交点之前，已对外接受资金写入后禁止恢复旧库覆盖事实。
...
schema v13 的命令与真实统计统一见《测试报告》。线上 Git SHA、冷备份目录、发布版本和冒烟结论只在真实发布后登记；旧 schema 记录不能替代当前门禁。
...
候选 release 验证阶段先执行通知配置与数据库 SQL 预检，再启动带门闩的 worker。候选 API、真实数据库、静态发布标记、同一 Git SHA、`schema_version=13`、通知预检及 Apache 配置全部通过后才开放入口和通知副作用。提交点之前失败恢复同一冷备集合；提交点之后绝不冷回滚已对外确认的资金写入。

只有用户明确授权验收环境全量替换时，才使用 `-SyncDemoDatabase` 同步经 `validate_v13_demo.py` 校验的 SQLite 快照，并同时同步匹配媒体。服务器保留 SMTP、JWT、机构凭据加密密钥等环境配置。远端执行 v13 迁移校验，健康检查必须同时返回目标 SHA 和 `schema_version=13`。
