<template>
  <div class="workspace-page">
    <section class="welcome-panel"><div><p>统一健康视图</p><h2>今天也要照顾好自己</h2><span>体检预约、预约状态、机构体检报告与日常自测自动汇入同一条时间线。</span></div><div class="welcome-actions"><el-button @click="router.push({name:'appointments'})">预约体检</el-button><el-button type="primary" @click="router.push({name:'measurements'})">记录日常测量</el-button></div></section>
    ...
    <el-card shadow="never" class="dashboard-card"><template #header><div class="card-heading"><div><h3>最近健康事件</h3><p>时间线中的最新记录</p></div><el-button link type="primary" @click="router.push({name:'timeline'})">查看全部</el-button></div></template>
      <el-timeline v-if="events.length"><el-timeline-item v-for="event in events" :key="`${event.type}-${event.occurred_at}-${event.item?.id}`" :timestamp="formatTime(event.occurred_at)"><strong>{{ event.title }}</strong></el-timeline-item></el-timeline><el-empty v-else description="暂无健康事件" />
    </el-card>
  </div>
</template>
<script setup>
...
const metrics=computed(()=>{const counts={self_measurement:0,institution_report:0,appointment:0};events.value.forEach(e=>{if(counts[e.type]!==undefined)counts[e.type]++});return[{label:"时间线事件",value:events.value.length,icon:"线",note:"统一健康记录"},{label:"体检预约",value:counts.appointment,icon:"约",note:"展示当前预约状态"},{label:"日常测量",value:counts.self_measurement,icon:"测",note:"保留全部原始值"},{label:"机构报告",value:counts.institution_report,icon:"报",note:"机构提交后永久归档"}]});
