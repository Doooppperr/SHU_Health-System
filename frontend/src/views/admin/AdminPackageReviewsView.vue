<template>
  <div class="workspace-page">
    ...
    <el-card shadow="never">
      <el-select v-model="status" placeholder="全部状态" @change="applyFilter"><el-option label="全部状态" value=""/><el-option label="待审核" value="pending"/><el-option label="已通过" value="approved"/><el-option label="已驳回" value="rejected"/><el-option label="已撤回" value="withdrawn"/></el-select>
...
async function load(){loading.value=true;try{const params={page:pagination.page,page_size:15};if(status.value)params.status=status.value;const{data}=await fetchAdminPackageChangeRequests(params);items.value=data.items||[];Object.assign(pagination,data.pagination||{});}finally{loading.value=false;}}
async function applyFilter(){pagination.page=1;await load();}
