...
Vue 3 + Vite 6 前端采用“公开机构目录 + 三角色工作台”的页面结构。当前基线为 schema v13，覆盖在线付款、资金进度、投诉与退款、机构收款与退款、平台财务，以及原有预约、报告、健康档案和 HealthDoc Agent。第八轮统一了三处智能搜索体验与访客顶部导航。

## 第八轮搜索与访客导航

- `SmartInstitutionSearch.vue` 由公开目录、登录目录和预约页共同复用：防抖后先请求内容搜索，后端提示需要意图理解时再请求混合搜索；请求版本号阻止旧响应覆盖新查询。
- 建议按机构、分院、套餐展示，支持关键词高亮、加载/降级/空状态、方向键、Enter、Esc、鼠标和触屏。点击建议只在当前页面过滤和突出理由，预约页仍要求用户主动选择分院。
