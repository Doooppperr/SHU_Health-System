...
康康健健 HealthDoc 是一个基于 Flask 与 Vue 3 的 B/S 健康管理项目。本地开发默认使用 SQLite schema v5。普通用户维护个人健康资料和日常自测，体检机构生产标准化报告；机构提交后，系统按注册用户的健康身份码与姓名即时永久归档。项目同时提供公开门户、机构服务、亲友只读授权、评论、OCR、健康 AI、关怀模式和三角色工作台。
...
- 机构账号：进入 `/org/dashboard`，维护所属机构资料、相册与套餐，通过手工或 OCR 创建报告，复核指标后锁定、提交并永久归档。一个机构可绑定多个同权限账号。
...
| 数据库 | 本地 SQLite schema v5；代码保留 GaussDB/openGauss 连接配置 |
...
新空库首次启动会直接创建 v5。已有非 v5 数据库不会被 `db.create_all()` 半升级；应用会提示执行升级脚本：
...
当前本地库已完成迁移，`PRAGMA user_version=5`、`integrity_check=ok`、外键违规为 0。升级前备份仍保留，不能在确认不需要前删除。
...
- 报告状态为 `draft → locked → published`。离开草稿后内容不可修改；提交后永久归档，不提供撤回、撤下或删除操作。
...
当前验收基线：后端 26 项、前端 15 个文件 85 项通过；Vite production build、Python/npm 依赖审计、44 条 RAG 黄金查询、SQLite 完整性/外键与全量快照迁移冒烟均通过。
...
- [数据库设计说明](项目文档/数据库设计说明.md)与[数据库规范化说明](项目文档/数据库规范化说明.md)：schema v5 物理模型、迁移、约束和范式。
