...
演示快照包含 25 个账号、5 个机构主体、15 家分院、25 个套餐、40 个预约组、56 位预约参与者及 66 份已归档体检报告。`test1` 对全部 104 个活动指标均至少有 9 条，核心趋势指标均有 16 条，覆盖 1458 天的四年故事（5 次综合体检、11 次专项复查），并包含正常、偏高、偏低、阳性/阴性、OCR/手工、多机构、同日优先级和规范附件批注。完整账号矩阵见[测试账号与演示数据](项目文档/测试账号与演示数据.md)。
...
当前验收基线：后端 95 项、前端 128 项测试通过，Vite production build 通过，Python 与前端生产依赖审计均无已知漏洞；SQLite schema v10、v9→v10 原子升级、openGauss v10 迁移、演示数据预检与覆盖流程均已验证。
