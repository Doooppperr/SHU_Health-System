...
康康健健 HealthDoc 是一个基于 Flask 与 Vue 3 的 B/S 健康服务平台，本地开发使用 SQLite schema v11。普通用户除原有健康档案、趋势、亲友和预约能力外，还可使用由 DeepSeek 规划、类型化工具执行、逐次确认和幂等回执保护的 HealthDoc Agent。平台保留旧 AI 回退，并提供混合 RAG、OAuth/MCP 与 OpenTelemetry 的灰度能力。
...
| 6.0 | LangGraph Agent、类型化工具、确认/幂等写入、混合 RAG、OAuth/MCP 和 OTel | 当前运行基线；数据库为 schema v11，新能力按开关灰度 |
...
- 普通用户：进入 `/dashboard`，通过“记录今日测量”抽屉录入六类日常数据；健康时间线同时呈现一次体检一张旅程卡和按自然日聚合的本人记录。健康趋势直接标注每个数据点，支持全部、近一周、近一个月、近半年和跨年自定义日期范围；只有具备可靠上下限的指标才展示简洁“参考范围”卡，身高、体重、腰围等描述性项目不展示判定提示。体检预约使用分步引导选择日期、机构、套餐和受检者，预约历史支持日期筛选并固定每页 10 组。
...
- HealthDoc Agent：DeepSeek 根据任务选择档案、趋势、机构、套餐、容量和预约类型化工具；数值与权限由代码处理。预约、取消、候补和人工客服只生成 10 分钟草稿，用户在第一方确认卡核对后才幂等提交。
...
| 数据库 | 本地 SQLite schema v11；GaussDB/openGauss 使用 Alembic v11 增量迁移 |
...
新空库首次启动会直接创建 v11。已有旧数据库不会被 `db.create_all()` 半升级；应用会提示执行原子升级脚本。v10→v11 只新增 Agent、人工工单和 OAuth 表；旧 v4–v10 SQLite 可由同一脚本逐级保留升级：
...
3. 在同目录临时文件创建完整 v11 结构；
4. 验证表、列、约束、外键和 `integrity_check`；
5. 生成带时间戳的升级前完整备份；
6. 原子替换正式数据库。

当前本地库为 `PRAGMA user_version=11`，`integrity_check=ok`、外键违规为 0。升级前备份仍需保留到人工验收结束。
...
验收快照包含 25 个账号、5 个机构主体、15 家分院、26 个套餐、40 个预约组、56 位预约参与者及 79 份已归档体检报告。全部 223 个“报告—健康方向”组合均有机构医生审核结论，1892 项报告指标均通过套餐、指标、附件和结论事实对齐校验。`institution1_staff1` 可完整只读打开同主体四家兄弟分院的 11 份归档档案，每份含 15—25 项套餐内指标和逐方向结论，其中 4 份保留轻度关注项。`test1` 保持 41 份报告，对全部 104 个活动指标均至少有 9 条，核心趋势指标均有 16 条，覆盖 1458 天（9 次综合体检、20 次专项或联合复查），并包含早期轻度代谢风险、中期改善、阶段性反弹、近期轻度体重关注，以及正常、偏高、偏低、阳性/阴性、OCR/手工、多机构、同日优先级和规范附件批注。完整账号矩阵见[验收账号与预置数据](项目文档/验收账号与预置数据.md)。
...
当前验收基线：后端 106 项、前端 139 项测试通过，Vite production build 通过；SQLite schema v10→v11 原子升级、openGauss v11 迁移、预置业务数据强校验、Agent/OAuth/MCP 安全契约均已验证。真实 DeepSeek 200 场景结果和线上 release 记录见[测试报告](项目文档/测试报告.md)。
...
仓库包含 Apache、Waitress、API/通知 worker、Agent 清理 timer、可选 MCP systemd 服务、openGauss 和带回滚的发布脚本。普通发布执行 v11 增量迁移并保留服务器数据；`-SyncDemoMedia` 只刷新清单素材；仅显式 `-SyncDemoDatabase` 才会在完整备份后导入验收快照。最新线上 release 和灰度状态见[服务器部署与同步](项目文档/服务器部署与同步.md)。
...
- [数据库设计说明](项目文档/数据库设计说明.md)与[数据库规范化说明](项目文档/数据库规范化说明.md)：schema v11 物理模型、迁移、约束和范式。
...
- [服务器部署与同步](项目文档/服务器部署与同步.md)：生产拓扑、schema v11 数据同步、备份、回滚和发布验收。
