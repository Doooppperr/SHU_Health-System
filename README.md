...
康康健健 HealthDoc 是一个基于 Flask 与 Vue 3 的 B/S 健康服务平台，当前数据库基线为 schema v12。本轮把访客目录、平台内实名认证、双向关联账号、健康身份码多人代预约、报告复核、投诉闭环、评论治理和机构经营画像纳入同一套服务端权限与审计模型；HealthDoc Agent 继续使用类型化工具、逐次确认和幂等回执。
...
| 第六轮验收 / schema v12 | 访客浏览、机构单账号、实名认证、关联账号、多来源预约、报告复核、投诉、评论治理和经营画像 | 当前开发与发布基线 |

版本演进不是多套并存系统。当前代码、测试、验收数据和文档共同描述 schema v12 平台，旧版本能力只在仍然有效时继续保留。
...
| 数据库 | 本地 SQLite schema v12；GaussDB/openGauss 使用 Alembic v12 增量迁移 |
...
新空库首次启动会直接创建 v12。已有旧数据库不会被 `db.create_all()` 半升级；应用会提示执行原子升级脚本。旧 v4—v11 SQLite 可由同一脚本逐级保留升级：
...
3. 在同目录临时文件创建完整 v12 结构并执行 v11→v12 语义转换；
4. 验证表、列、约束、外键和 `integrity_check`；
5. 生成带时间戳的升级前完整备份；
6. 原子替换正式数据库。

升级成功条件为 `PRAGMA user_version=12`、`integrity_check=ok` 且外键违规为 0。升级前备份必须保留到人工验收结束。

重建本地验收数据时，先只读检查，再显式确认覆盖业务验收记录。当前统一使用 `reset_v12_demo_data.py`；旧 `reset_v10_demo_data.py` 仅为历史命令兼容包装。完成后必须对显式数据库与上传目录运行 v12 强校验器：
...
| 角色 | 登录后入口 | 当前权限 |
|---|---|---|
| 普通用户 `user` | `/dashboard` | 一次性实名认证、自测、当前有效账号的健康数据、关联账号切换、多人预约、投诉、评论和健康 AI |
| 机构账号 `institution_admin` | `/org/dashboard` | 单一绑定分院的预约履约、报告上传/复核、投诉处理、经营画像和套餐审核申请 |
| 系统管理员 `admin` | `/admin/dashboard` | 分院及唯一机构账号、用户身份修正、投诉闭环、评论处罚/申诉；无健康档案内容 |
...
验收快照有 23 个启用可登录账号、5 个机构主体和 15 家分院；重复机构账号可作为停用解绑的历史行保留。`test1` 继续覆盖全部 104 个活动指标和长期趋势；v12 另覆盖三层关联链、三类预约参与人、身高体重两项/一项/均缺、代理开关关闭、报告三态、投诉五态、评论处罚与申诉、机构画像分布。精确行数由只读 `validate_v12_demo.py` 输出，完整矩阵见[验收账号与预置数据](项目文档/验收账号与预置数据.md)。
...
- 关联亲友接受后成为双向关系，可建立最多三层的受控关联会话；服务端持续记录真实操作者、当前有效账号和完整链。撤销、停用或安全版本变化立即使下游会话失效。
...
- 机构报告采用 `draft → pending_review → published`。上传确认与复核确认分别记录虚构/真实业务医生姓名及时间，发布事务同时锁档、履约、写事件和通知，之后不可修改或撤回。
- 每个个人预约最多一条投诉，支持机构回复、用户确认或随时升级平台；评论隐藏可附带 7/30 天或永久禁言，处罚仅影响发言并允许一次申诉。
- 趋势页单列最近异常及历史异常；机构首页只用匿名聚合数据生成性别、年龄、套餐画像和 AI 建议。
...
- [数据库设计说明](项目文档/数据库设计说明.md)与[数据库规范化说明](项目文档/数据库规范化说明.md)：schema v12 物理模型、迁移、约束和范式。
...
- [服务器部署与同步](项目文档/服务器部署与同步.md)：生产拓扑、schema v12 数据同步、冷备份、回滚和发布验收。
