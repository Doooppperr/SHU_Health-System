...
康康健健 HealthDoc 是一个基于 Flask 与 Vue 3 的 B/S 健康服务平台，当前数据库基线为 schema v13。当前发布基线在第八轮混合智能搜索之上，进一步锁定机构身份资料、扩充机构经营画像验收样本，并支持 `healthdoc.cn`、`www.healthdoc.cn` 与原公网 IP 并行访问；公开页面和工作台统一展示工信部备案链接。
...
| 第七轮优化 / schema v13 | 付款订单、平台托管、2.5% 服务费、七日结算、投诉退款、72 小时机构退款与运营暂停 | 当前数据库与资金基线 |
| 第八轮优化 / schema v13 | 多字段内容召回、按需 DeepSeek 意图重排、可解释推荐、三处统一搜索和访客导航修复 | 当前功能与发布基线 |
| 2026-08 运营修复 / schema v13 | 机构身份锁定、画像样本扩充、域名/HTTPS 与备案信息 | 当前运维与合规基线 |
...
- 机构账号：每个分院只有一个有效账号，由管理员创建分院时一并设置用户名、初始密码和邮箱。机构主体、分院名称、所在区域和详细地址属于身份资料，创建后机构账号不可自行修改，平台管理员仍可纠错；电话、交通、分机、轮休日和简介可继续维护。凭据邮件可重试，发送成功后清除发件箱中的加密临时凭据。机构可在无未结业务时软注销，管理员可恢复。
...
- 智能搜索：三处机构搜索共用内容召回服务，覆盖机构、分院、地区、地址、交通、服务特点及在售套餐的人群、健康方向和描述。输入先返回 `content` 结果；只有复杂表达或低置信度时才按需请求 `hybrid`，DeepSeek 只接收最长 80 字的本次查询并输出受控意图。模型异常时以 `content_fallback` 和 HTTP 200 降级，不保存搜索历史，不读取个人资料。
- 公网与备案：生产入口为 `https://healthdoc.cn` 与 `https://www.healthdoc.cn`；IP 的 80 端口根路径为空白页，`http://111.229.87.94/healthdoc/` 提供 HealthDoc 子路径构建，`http://111.229.87.94/cs2_inventory/` 提供独立 CS2 Inventory 服务。公开页和三角色工作台展示“沪ICP备2026034136号-1”，并链接到工信部备案系统。
...
验收快照有 23 个启用可登录账号、5 个机构主体和 15 家分院；另有 12 个明确不可登录的经营画像样本受检者，每个启用分院各有一组近期已发布演示报告，覆盖男女与五个成人年龄段。`test1` 继续覆盖全部 104 个活动指标和长期趋势。v13 另包含 11 笔付款订单、全部六种资金状态、5 个退款案件、72 小时待退款及逾期暂停场景。精确结果由只读 `validate_v13_demo.py` 输出，完整矩阵见[验收账号与预置数据](项目文档/验收账号与预置数据.md)。
...
本轮测试、构建、依赖审计、v13 数据强校验、浏览器关键路径和发布恢复门禁已于 2026-08-04 收口，真实统计统一见[测试报告](项目文档/测试报告.md)，README 不重复维护易失效数字。最终 Git SHA 与线上发布结果只按真实执行结果登记。
