...
康康健健 HealthDoc 是一个基于 Flask 与 Vue 3 的 B/S 健康服务平台，当前数据库基线为 schema v13。第八轮在不变更数据库结构和业务数据的前提下，将公开目录、登录目录与预约选院统一升级为可解释的混合智能搜索，并修复访客顶部导航在宽屏、窄屏、深浅主题和关怀模式下的布局一致性。
