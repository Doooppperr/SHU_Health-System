...
Flask 后端负责公开认证、三角色授权、个人健康资料、日常测量、机构报告生产与永久归档、机构运营、系统管理、亲友只读授权、评论、OCR 和健康 AI。本地使用 SQLite schema v5；服务器通过 `DATABASE_URL` 连接 GaussDB/openGauss，并支持从已验证 SQLite 快照执行显式全量同步。
...
默认数据库为 `instance/health_system.db`。仓库跟踪这一份只含合成数据的演示快照，包含 5 个普通用户的丰富时间线；账号见 [`../项目文档/测试账号与演示数据.md`](../项目文档/测试账号与演示数据.md)。SQLite 连接启用外键；`PRAGMA user_version=5` 标识当前结构。新空库会直接创建 v5，非空旧库版本不匹配时拒绝启动。
...
可分析对象为本人或已授权亲友的 `published` 机构报告。精确 `selected_record_ids` 与 `record_scope: {"owner_id": 2, "mode": "all_confirmed"}` 互斥，后者在 schema v5 中解析为该归属人的全部已发布报告；两种方式都必须逐请求同意并重新鉴权。
...
当前 26 项测试使用独立内存 SQLite，不修改 `instance/health_system.db`；覆盖 schema v5、v4 全量保留升级、丰富演示时间线、机构报告永久归档与身份失败回滚、亲友只读边界、趋势/隐私、AI 同意、RAG 切分/同步/重排/安全降级、OCR Mock 与文件删除，以及全量快照迁移。完整结论见 [`../项目文档/测试报告.md`](../项目文档/测试报告.md)。
